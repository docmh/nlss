...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\run_rscript.ps1 scripts\crosstabs.R --csv data.csv --row gender --col condition --out .\outputs\tmp
```

```powershell
powershell -ExecutionPolicy Bypass -File scripts\run_rscript.ps1 scripts\crosstabs.R --csv data.csv --rows gender,handedness --cols condition
```
...
- `--out`: Output directory (default: `./outputs/tmp`, hard-coded when omitted, relative to the working directory).
