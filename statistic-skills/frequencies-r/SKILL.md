...
```powershell
powershell -ExecutionPolicy Bypass -File ..\run_rscript.ps1 scripts\frequencies.R --csv data.csv --vars gender,condition --out ..\outputs\tmp
```

```powershell
powershell -ExecutionPolicy Bypass -File ..\run_rscript.ps1 scripts\frequencies.R --csv data.csv --vars gender --group condition
```
...
- `--out` sets the output directory (default: `statistic-skills/outputs/tmp`).
