...
print_usage <- function() {
  ...
  cat("  --out DIR              Output directory (default: statistic-skills/outputs/tmp)\n")
...
get_default_out <- function() {
  ...
  if (length(file_arg) > 0) {
    ...
    return(normalizePath(file.path(script_dir, "..", "..", "outputs", "tmp"),
                         winslash = "/", mustWork = FALSE))
  }
  normalizePath(file.path("statistic-skills", "outputs", "tmp"),
                winslash = "/", mustWork = FALSE)
}
