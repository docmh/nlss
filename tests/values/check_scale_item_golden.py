...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_scale_item_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    row = find_row(item_df, expected)
    if row is None:
        fail("Expected item row not found")
