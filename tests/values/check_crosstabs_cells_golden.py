...
def find_row(cells_df, expected):
    ...
    exp_row_level = normalize_text(expected.get("row_level"))
    exp_col_level = normalize_text(expected.get("col_level"))
    ...
    for row in cells_df:
        ...
        if normalize_group(row.get("group")) != exp_group:
            continue
        if normalize_text(row.get("row_level")) != exp_row_level:
            continue
        if normalize_text(row.get("col_level")) != exp_col_level:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_crosstabs_cells_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    cells_df = results.get("cells_df") or []
    if not cells_df:
        fail("cells_df is empty")

    row = find_row(cells_df, expected)
    if row is None:
        fail("Expected cell row not found")
