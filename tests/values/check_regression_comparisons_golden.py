...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_regression_comparisons_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No regression log entry found for comparisons")

    results = entry.get("results", {}) or {}
    comparisons_df = results.get("comparisons_df") or []
    if not comparisons_df:
        fail("comparisons_df is empty")

    row = find_row(comparisons_df, expected)
    if row is None:
