...
def find_row(summary_df, expected):
    exp_step = normalize_text(expected.get("step"))
    for row in summary_df:
        if exp_step and normalize_text(row.get("step")) != exp_step:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_sem_invariance_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if not fit:
        fail("Invariance fit table is empty")

    row = find_row(fit, expected)
    if row is None:
        fail("Expected invariance row not found")

    numeric_keys = ["chisq", "df", "p", "cfi", "tli", "rmsea", "srmr", "delta_cfi", "delta_rmsea"]
