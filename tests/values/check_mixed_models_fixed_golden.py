...
def match_options(options, expected):
    ...
    exp_std = normalize_text(expected.get("standardize"))
    ...
    if exp_std:
        actual_std = normalize_text(options.get("standardize")) or "none"
        if exp_std != actual_std:
            return False
...
def find_row(fixed_df, expected):
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_mixed_models_fixed_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No mixed_models log entry found for fixed effects check")

    results = entry.get("results", {}) or {}
    fixed_df = results.get("fixed_effects_df") or []
    if not fixed_df:
        fail("fixed_effects_df is empty")

    row = find_row(fixed_df, expected)
    if row is None:
        fail("Expected fixed effects row not found")

    numeric_keys = ["estimate", "se", "df", "t", "p", "ci_low", "ci_high", "std_beta"]
