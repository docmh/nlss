...
def find_row(diagnostics_df, expected):
    ...
    exp_test = normalize_text(expected.get("test"))
    for row in diagnostics_df:
        if normalize_text(row.get("model")) != exp_model:
            continue
        if exp_test and normalize_text(row.get("test")) != exp_test:
            continue
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_regression_diagnostics_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No regression log entry found for diagnostics")
