...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_crosstabs_diagnostics_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    diag_df = results.get("diagnostics_df") or []
    if not diag_df:
        fail("diagnostics_df is empty")

    row = find_row(diag_df, expected)
    if row is None:
