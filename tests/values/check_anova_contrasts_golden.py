...
def find_row(contrasts_df, golden_row):
    term = normalize_text(golden_row.get("term"))
    contrast = normalize_text(golden_row.get("contrast"))
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_anova_contrasts_golden.py <log_path> <start_count> <golden_csv> <case_id> [mode]")
    ...
    if entry is None:
        fail("No anova log entry found for contrasts check")
    ...
    if row is None:
        fail("Expected contrast row not found")
