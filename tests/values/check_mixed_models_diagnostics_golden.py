...
def find_row(diagnostics_df, expected):
    metric = normalize_text(expected.get("metric"))
    for row in diagnostics_df:
        if normalize_text(row.get("metric")) == metric:
            return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_mixed_models_diagnostics_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No mixed_models log entry found for diagnostics check")
    ...
    expected_value = normalize_text(expected.get("value"))
    if expected_value:
        if normalize_text(row.get("value")) != expected_value:
            fail(f"Mismatch for value: expected {expected_value}, got {row.get('value')}")

    compare_numeric(row.get("statistic"), parse_float(expected.get("statistic")), "statistic")
    compare_numeric(row.get("p"), parse_float(expected.get("p")), "p")
