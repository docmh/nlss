...
def find_row(emmeans_df, expected):
    term = normalize_text(expected.get("term"))
    level = normalize_text(expected.get("level"))
    for row in emmeans_df:
        if normalize_text(row.get("term")) != term:
            continue
        if normalize_text(row.get("level")) != level:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_mixed_models_emmeans_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No mixed_models log entry found for emmeans check")

    results = entry.get("results", {}) or {}
    emmeans_df = results.get("emmeans_df") or []
    if not emmeans_df:
        fail("emmeans_df is empty")

    row = find_row(emmeans_df, expected)
    if row is None:
        fail("Expected emmeans row not found")

    numeric_keys = ["emmean", "se", "df", "t", "p", "ci_low", "ci_high"]
