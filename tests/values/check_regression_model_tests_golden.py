...
def find_row(model_tests_df, expected):
    exp_model = normalize_text(expected.get("model"))
    exp_group = normalize_text(expected.get("group"))
    exp_source = normalize_text(expected.get("source"))
    for row in model_tests_df:
        if normalize_text(row.get("model")) != exp_model:
            continue
        if normalize_text(row.get("source")) != exp_source:
            continue
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_regression_model_tests_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if entry is None:
        fail("No regression log entry found for model tests")

    results = entry.get("results", {}) or {}
    model_tests_df = results.get("model_tests_df") or []
    if not model_tests_df:
        fail("model_tests_df is empty")

    row = find_row(model_tests_df, expected)
    if row is None:
        fail("Expected model tests row not found")

    numeric_keys = ["ss", "df", "ms", "f", "p", "deviance", "chisq"]
