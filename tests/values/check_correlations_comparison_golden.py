...
def find_row(comparison_df, expected):
    exp_var1 = normalize_text(expected.get("var1"))
    exp_var2 = normalize_text(expected.get("var2"))
    exp_group1 = normalize_text(expected.get("group1"))
    exp_group2 = normalize_text(expected.get("group2"))
    for row in comparison_df:
        if normalize_text(row.get("var1")) != exp_var1:
            continue
        if normalize_text(row.get("var2")) != exp_var2:
            continue
        if normalize_text(row.get("group1")) != exp_group1:
            continue
        if normalize_text(row.get("group2")) != exp_group2:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_correlations_comparison_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    if not comparison_df:
        fail("comparison_df is empty")

    row = find_row(comparison_df, expected)
    if row is None:
        fail("Expected comparison row not found")
