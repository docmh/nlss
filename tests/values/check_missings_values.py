...
"""Compare legacy missings JSONL output to independent base-R golden cells."""
import csv
import json
import math
import sys
...
def main():
    if len(sys.argv) != 5:
        raise SystemExit("Usage: check_missings_values.py LOG START_COUNT GOLDEN_CSV CASE_ID")
    ...
    entries = [entry for entry in entries if entry.get("module") == "missings"]
    if not entries:
        raise AssertionError("No new missings JSONL entry")
    ...
    print(f"[PASS] missings golden {case_id}: {len(expected)} independent cells")
