...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_sem_r2_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    row = find_row(r2_df, expected)
    if row is None:
        fail("Expected R2 row not found")

    expected_val = parse_float(expected.get("r2"))
    compare_numeric(row.get("r2"), expected_val, "r2")
