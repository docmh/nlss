...
def find_row(mod_df, expected):
    exp_lhs = normalize_text(expected.get("lhs"))
    exp_op = normalize_text(expected.get("op"))
    exp_rhs = normalize_text(expected.get("rhs"))
    for row in mod_df:
        if exp_lhs and normalize_text(row.get("lhs")) != exp_lhs:
            continue
        if exp_op and normalize_text(row.get("op")) != exp_op:
            continue
        if exp_rhs and normalize_text(row.get("rhs")) != exp_rhs:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_sem_modindices_golden.py <log_path> <start_count> <golden_csv> <case_id>")
    ...
    mod_df = options.get("modindices") or []
    if not mod_df:
        fail("modindices output is empty")

    row = find_row(mod_df, expected)
    if row is None:
        fail("Expected modindices row not found")

    numeric_keys = ["mi", "epc", "sepc.all"]
