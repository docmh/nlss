...
def find_row(posthoc_df, golden_row):
    term = normalize_text(golden_row.get("term"))
    group = normalize_text(golden_row.get("group"))
    group_1 = normalize_text(golden_row.get("group_1"))
    group_2 = normalize_text(golden_row.get("group_2"))
    method = normalize_text(golden_row.get("method"))
    for row in posthoc_df:
        if normalize_text(row.get("term")) != term:
            continue
        if normalize_text(row.get("group")) != group:
            continue
        if normalize_text(row.get("group_1")) != group_1:
            continue
        if normalize_text(row.get("group_2")) != group_2:
            continue
        if method and normalize_text(row.get("method")) != method:
            continue
        return row
    return None
...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_anova_posthoc_golden.py <log_path> <start_count> <golden_csv> <case_id> [mode]")
    ...
    if entry is None:
        fail("No anova log entry found for posthoc check")
