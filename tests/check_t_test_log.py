...
def main():
    if len(sys.argv) < 5:
        fail("Usage: check_t_test_log.py LOG START STATUS MODE [KEY=VALUE...]")
    ...
    for candidate in reversed(entries):
        if candidate.get("module") == "t_test":
            entry = candidate
            break
    if entry is None:
        fail("No t_test log entry found.")
    ...
    if expected_mode not in ("", "-", "none", "null"):
