...
- Defaults are loaded from `nlss/scripts/config.yml`; CLI flags override config values.
...
The NLSS format output uses `nlss/assets/init-workspace/default-template.md` by default. The path can be overridden via `templates.init_workspace.default` in `nlss/scripts/config.yml`.
