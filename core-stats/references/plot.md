...
Use the Markdown template at `core-stats/assets/plot/default-template.md` when assembling plot reports. If the template exists, `plot.R` uses it for `apa_report.md`.

- The template path can be overridden via `templates.plot.default` in `core-stats/scripts/config.yml`.
