...
CHECK_SCRIPT="${ROOT_DIR}/outputs/tests/check_t_test_log.py"
...
dataset_apa() {
  local path="$1"
  echo "$(dataset_dir "${path}")/apa_report.md"
}
...
CSV_PAIRED_TINY_APA_PATH="$(dataset_apa "${CSV_PAIRED_TINY_PATH}")"
CSV_SMALL_GROUPS_APA_PATH="$(dataset_apa "${CSV_SMALL_GROUPS_PATH}")"
CSV_LOG_OFF_APA_PATH="$(dataset_apa "${CSV_LOG_OFF_PATH}")"
CSV_LOG_OFF_LOG_PATH="$(dataset_log "${CSV_LOG_OFF_PATH}")"
CSV_USER_PROMPT_LOG_PATH="$(dataset_log "${CSV_USER_PROMPT_PATH}")"
CSV_USER_PROMPT_APA_PATH="$(dataset_apa "${CSV_USER_PROMPT_PATH}")"
RDATA_LABEL="ttest_rdata"
RDATA_DIR="${WORKSPACE_DIR}/${RDATA_LABEL}"
RDATA_LOG_PATH="${RDATA_DIR}/analysis_log.jsonl"
RDATA_APA_PATH="${RDATA_DIR}/apa_report.md"
...
run_ok "interactive one-sample" env CORE_STATS_PROMPT_FILE="${INTERACTIVE_INPUT}" Rscript "${R_SCRIPT_DIR}/t_test.R" --interactive
...
require_file "${MAIN_APA_PATH}"
require_file "${MAIN_LOG_PATH}"
assert_contains "${MAIN_APA_PATH}" "t-test results."
assert_contains "${MAIN_APA_PATH}" "**Narrative**"
...
assert_contains "${MAIN_APA_PATH}" "Bootstrap CIs use 1000 resamples."
...
assert_contains "${SMALL_APA_PATH}" "could not be computed"
...
assert_contains "${CSV_SMALL_GROUPS_APA_PATH}" "could not be computed"
...
assert_contains "${CSV_PAIRED_TINY_APA_PATH}" "could not be computed"
...
require_file "${CSV_LOG_OFF_APA_PATH}"
...
require_file "${CSV_USER_PROMPT_APA_PATH}"
...
check_log "${MAIN_LOG_PATH}" "${start}" "-" "one_sample"
assert_contains "${MAIN_APA_PATH}" "T_TEST_TEMPLATE_DEFAULT"
