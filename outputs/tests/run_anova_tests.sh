...
start=$(log_count "${LOG_PATH_CSV_SEMI}")
run_ok "anova interactive between" env CORE_STATS_PROMPT_FILE="${INTERACTIVE_INPUT}" Rscript "${R_SCRIPT_DIR}/anova.R" --interactive
...
check_anova_log "${start}" "between" "none" "0" "partial_eta_sq" "-" "-" type="${EXPECTED_TYPE_II}" || exit 1
assert_contains "${APA_REPORT_PATH}" "ANOVA_TEMPLATE_DEFAULT"
...
check_anova_log "${start}" "between" "tukey" "gt0" "partial_eta_sq" "-" "-" type="${EXPECTED_TYPE_II}" || exit 1
assert_contains "${APA_REPORT_PATH}" "ANOVA_TEMPLATE_POSTHOC"
