...
CUSTOM_REPORT_PATH="${CUSTOM_DIR}/apa_report.md"
...
TYPED_RDS_REPORT_PATH="${TYPED_RDS_DIR}/apa_report.md"
TYPED_RDATA_DIR="${WORKSPACE_DIR}/${TYPED_DF_NAME}"
TYPED_RDATA_LOG_PATH="${TYPED_RDATA_DIR}/analysis_log.jsonl"
TYPED_RDATA_REPORT_PATH="${TYPED_RDATA_DIR}/apa_report.md"
...
SEMI_REPORT_PATH="${SEMI_DIR}/apa_report.md"
...
NO_HEADER_REPORT_PATH="${NO_HEADER_DIR}/apa_report.md"
...
ROUNDING1_REPORT_PATH="${ROUNDING1_DIR}/apa_report.md"
...
ROUNDING2_REPORT_PATH="${ROUNDING2_DIR}/apa_report.md"
...
SAV_REPORT_PATH="${SAV_DIR}/apa_report.md"
...
check_log_value() {
  ...
  python3 - "${log_path}" "${start_count}" "${path}" "${expected}" <<'PY'
import json
import sys
from pathlib import Path
...
check_overview_value() {
  ...
  local expected="$1"; shift
  python3 - "${log_path}" "${start_count}" "${variable}" "${column}" "${expected}" <<'PY'
import json
import sys
from pathlib import Path
...
check_levels_value() {
  ...
  local column="$1"; shift
  local expected="$1"; shift
  python3 - "${log_path}" "${start_count}" "${variable}" "${level}" "${column}" "${expected}" <<'PY'
import json
import sys
from pathlib import Path
...
check_levels_absent() {
  ...
  local start_count="$1"; shift
  local variable="$1"; shift
  python3 - "${log_path}" "${start_count}" "${variable}" <<'PY'
import json
import sys
from pathlib import Path
...
assert_contains "${APA_REPORT_PATH}" "Data Exploration"
assert_contains "${APA_REPORT_PATH}" "Variable overview."
assert_contains "${APA_REPORT_PATH}" "Value levels."
assert_contains "${APA_REPORT_PATH}" "Table 1"
assert_contains "${APA_REPORT_PATH}" "Table 2"
...
start=$(log_count "${CUSTOM_LOG_PATH}")
run_ok "interactive csv" env CORE_STATS_PROMPT_FILE="${INTERACTIVE_CSV_INPUT}" Rscript "${R_SCRIPT_DIR}/data_explorer.R" --interactive
...
start=$(log_count "${TYPED_RDATA_LOG_PATH}")
run_ok "interactive rdata" env CORE_STATS_PROMPT_FILE="${INTERACTIVE_RDATA_INPUT}" Rscript "${R_SCRIPT_DIR}/data_explorer.R" --interactive
...
run_expect_fail "interactive invalid input" "${LOG_PATH}" \
  env CORE_STATS_PROMPT_FILE="${INTERACTIVE_BAD_INPUT}" Rscript "${R_SCRIPT_DIR}/data_explorer.R" --interactive
...
assert_contains "${APA_REPORT_PATH}" "Data Explorer TEMPLATE OVERRIDE"
assert_contains "${APA_REPORT_PATH}" "Overview Override."
assert_contains "${APA_REPORT_PATH}" "Levels Override."
assert_contains "${APA_REPORT_PATH}" "*Note-OVR.*"
assert_contains "${APA_REPORT_PATH}" "**Narrative Override**"
assert_contains "${APA_REPORT_PATH}" "OVERRIDE NARRATIVE:"
assert_contains "${APA_REPORT_PATH}" "| Var | Scale |"
assert_contains "${APA_REPORT_PATH}" "| Var | Level | n |"
assert_not_contains "${APA_REPORT_PATH}" "DropMe"
assert_not_contains "${APA_REPORT_PATH}" "DropLevel"
