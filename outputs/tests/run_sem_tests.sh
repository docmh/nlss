...
check_invariance_rows() {
  ...
  python3 - "${LOG_PATH}" "${start_count}" "${min_rows}" <<'PY'
import json
import sys
from pathlib import Path
...
assert_contains "${APA_REPORT_PATH}" "indirect := a*b"
...
assert_contains "${APA_REPORT_PATH}" "SEM_TEMPLATE_DEFAULT"
...
assert_contains "${APA_REPORT_PATH}" "SEM_TEMPLATE_MEDIATION"
...
assert_contains "${APA_REPORT_PATH}" "SEM_TEMPLATE_INVARIANCE"
