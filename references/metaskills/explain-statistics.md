...
- If a formal report is requested, write `report_<YYYYMMDD>_explain-statistics_<intent>.md` (ASCII slug for `<intent>`), align it using `references/metaskills/formatting/align-report.md`, then append a brief `# Synopsis` to `report_canonical.md` manually (no `metaskill-runner`).
